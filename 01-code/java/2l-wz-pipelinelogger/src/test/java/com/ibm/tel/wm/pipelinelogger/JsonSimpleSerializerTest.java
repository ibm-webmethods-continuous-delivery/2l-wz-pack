...
/**
 * Unit tests for JsonSimpleSerializer.
 * Tests simplified JSON serialization without type information, password masking, and error handling.
 */
public class JsonSimpleSerializerTest {
    
    private JsonSimpleSerializer serializer;
    ...
    public void testIsEnabled_WhenConfigEnabled_ReturnsTrue() {
        Config.INSTANCE.setJsonSimpleSerializerEnabled(true);
        assertTrue(serializer.isEnabled());
    }
    
    @Test
    public void testIsEnabled_WhenConfigDisabled_ReturnsFalse() {
        Config.INSTANCE.setJsonSimpleSerializerEnabled(false);
    ...
    @Test
    public void testSerialize_WithNullValue_SerializesAsNull() throws Exception {
    ...
    @Test
    public void testSerialize_WithStringValue_SerializesDirectly() throws Exception {
        ...
        assertEquals("john.doe", username.asText());
        // Should NOT have type information
        assertFalse(username.isObject());
    }
    ...
    public void testSerialize_WithPasswordField_MasksValue() throws Exception {
        ...
        assertEquals("*", inputPipeline.get("password").asText());
        assertEquals("*", inputPipeline.get("userPassword").asText());
        assertEquals("*", inputPipeline.get("dbPassword").asText());
    ...
    public void testSerialize_WithStringArray_SerializesAsArray() throws Exception {
        ...
        assertEquals("value1", stringArray.get(0).asText());
        assertEquals("value2", stringArray.get(1).asText());
        assertEquals("value3", stringArray.get(2).asText());
    }
    ...
    public void testSerialize_WithPasswordArray_MasksAllValues() throws Exception {
        ...
        assertTrue(passwordsArray.isArray());
        assertEquals(3, passwordsArray.size());
        
        for (int i = 0; i < 3; i++) {
            assertEquals("*", passwordsArray.get(i).asText());
        }
    ...
    public void testSerialize_With2DStringArray_SerializesCorrectly() throws Exception {
        ...
        assertTrue(matrixNode.isArray());
        assertEquals(2, matrixNode.size());
        
        JsonNode row0 = matrixNode.get(0);
        assertTrue(row0.isArray());
        assertEquals("a1", row0.get(0).asText());
        assertEquals("a2", row0.get(1).asText());
        
        JsonNode row1 = matrixNode.get(1);
    ...
    public void testSerialize_WithNestedIData_SerializesRecursively() throws Exception {
        ...
        JsonNode nestedField = parent.get("nestedField");
        assertTrue(nestedField.isTextual());
        assertEquals("nestedValue", nestedField.asText());
    }
    ...
    public void testSerialize_WithIDataArray_SerializesArray() throws Exception {
        ...
        JsonNode item1Node = items.get(0);
        assertTrue(item1Node.isObject());
        assertEquals("1", item1Node.get("id").asText());
        
        JsonNode item2Node = items.get(1);
        assertTrue(item2Node.isObject());
        assertEquals("2", item2Node.get("id").asText());
    }
    ...
    public void testSerialize_WithByteArray_MasksValue() throws Exception {
        ...
        assertTrue(binaryData.isTextual());
        assertEquals("*", binaryData.asText());
    }
    ...
    public void testSerialize_WithDateValue_FormatsAsISO8601() throws Exception {
        ...
        String dateValue = timestamp.asText();
    ...
    @Test
    public void testSerialize_WithObjectArray_SerializesAsStringArray() throws Exception {
        Object[] objects = {
            "string",
            Integer.valueOf(42),
            null
        ...
        assertTrue(mixedArray.isArray());
        assertEquals(3, mixedArray.size());
        
        assertEquals("string", mixedArray.get(0).asText());
        assertEquals("42", mixedArray.get(1).asText());
        assertTrue(mixedArray.get(2).isNull());
    }
    ...
    public void testSerialize_WithBothPipelines_SerializesBoth() throws Exception {
        ...
        assertEquals("inputValue", inputPipeline.get("input").asText());
        
        JsonNode outputPipeline = json.get("outputPipeline");
        assertNotNull(outputPipeline);
        assertEquals("outputValue", outputPipeline.get("output").asText());
    }
    ...
    @Test
    public void testSerialize_NoTypeInformation() throws Exception {
        ...
        assertFalse(result.contains("java.lang.String"));
        assertFalse(result.contains("java.lang.Integer"));
        
        JsonNode json = mapper.readTree(result);
        JsonNode inputPipeline = json.get("inputPipeline");
        
        // Values should be direct, not wrapped in objects
        assertTrue(inputPipeline.get("stringField").isTextual());
        assertTrue(inputPipeline.get("numberField").isTextual());
    }
    ...
    public void testPasswordFieldDetection_CaseInsensitive() throws Exception {
        ...
        assertEquals("*", inputPipeline.get("Password").asText());
        assertEquals("*", inputPipeline.get("pAsSwOrD").asText());
        assertEquals("*", inputPipeline.get("myPasswordField").asText());
    }
    ...
    public void testSerialize_WithSerializationError_HandlesGracefully() throws Exception {
        ...
        String value = problematicNode.asText();
        assertTrue(value.contains("Serialization error"));
    }
    
    @Test
    public void testSerialize_SimplerThanCompact() throws Exception {
        IDataUtil.put(testPipeline.getCursor(), "field", "testdata");
        
        String result = serializer.serialize("test.service", 100L, testPipeline, IDataFactory.create());
        
        JsonNode json = mapper.readTree(result);
        JsonNode inputPipeline = json.get("inputPipeline");
        JsonNode field = inputPipeline.get("field");
        
        // In simple serializer, values are direct (not wrapped in type/value objects)
        assertTrue(field.isTextual());
        assertEquals("testdata", field.asText());
        
        // Should be shorter/simpler than compact format - no type/value wrapper objects
        assertFalse(result.contains("\"type\":"));
